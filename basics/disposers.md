...
* For resources required while a dialog is displayed, use `DialogWrapper.getDisposable`.
* For resources required while a toolwindow tab is displayed, pass your instance implementing `Disposable` to 
`Content.setDisposer`.
...
The flexibility of the `Disposer` API means that if the parent instance is chosen unwisely, the child may consume resources for a longer period of time than is required. 
...
the entire component will remain in memory after the operation is complete, which creates memory pressure and can waste
CPU cycles on processing events which are no longer relevant for anything.
...
The same rules apply to message bus connections. Always pass a parent disposable to `MessageBus.connect`, and make sure
