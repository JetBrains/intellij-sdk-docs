...
Executes plugin tests against the currently selected IntelliJ Platform with the built plugin loaded.
It extends the [`Test`][gradle-test-task] Gradle task, enabling extensive configuration options such as system properties and memory management.

This task class also inherits from [`CustomIntelliJPlatformVersionAware`](tools_intellij_platform_gradle_plugin_task_awares.md#CustomIntelliJPlatformVersionAware), which makes it possible to create `testIde`-like tasks with custom IntelliJ Platform versions:
