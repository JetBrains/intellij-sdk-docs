...
```
or if you don't use IntelliJ Gradle Platform Plugin:
```
// Starter Framework
...
Note: While `LATEST-EAP-SNAPSHOT` is available, you might prefer using specific release versions to ensure test stability. The framework follows the IntelliJ IDEA versioning scheme, for example:
`testImplementation("com.jetbrains.intellij.tools:ide-starter-squashed:243.23654.153")`.
