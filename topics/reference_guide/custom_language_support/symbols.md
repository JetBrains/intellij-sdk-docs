...
> This API is available starting from 2020-2016 and is currently in development and thus in an experimental state.
>
{type="warning"}
...
The IntelliJ Platform uses [`🔝🅿️`](upsource:///platform/core-api/src/com/intellij/model/Symbol.java) to represent symbols, and `Symbol` serves as a link between Platform APIs, such as navigation, finding usages, or renaming.
...
`Symbol` decouples semantic actions from [FW4248](FW1342.md).
...
- Compiled class is `📥 in 📑model`, it's backed by JDK library stubs, and it's not bound to any project.
