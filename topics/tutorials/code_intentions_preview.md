[//]: # (title: Intention Action Preview)
...
- The code changed in several unrelated lines. The preview will then show several parts without then unchanged lines in the middle and separate the parts nicely.
...
- Verify that `getElementToMakeWritable()` returns its argument.
- Check if all instance fields in the class are either marked as `@SafeFieldForPreview` (defined in
...
- Your action modifies the results of `ReferencesSearch.search(declaration)`, which always belongs to the original physical file, no matter if the passed declaration is a copy or an original.
